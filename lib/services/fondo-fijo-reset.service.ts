...
export interface ResetConfig {
  acumular_pendientes_con_fijo?: boolean;
  forzar_reset?: boolean;
}

export interface ResetResult {
  exito: boolean;
  mensaje: string;
  fondosAfectados: number;
  detalles: string[];
}
  ...
  static async ejecutarResetAutomatico() {
    try {
          ...
          OR: [
              ...
              AND: [
                { dias_restablecimiento: { gt: 0 } },
                {
                  ultima_fecha_restablecimiento: {
                    lte: this.calcularFechaLimite(hoy, 'individual'),
                  },
                },
              ],
            },
            // Fondos sin configuración individual (usar global)
            {
              AND: [
                ...
                {
                  ultima_fecha_restablecimiento: {
                    lte: this.calcularFechaLimite(hoy, 'global', diasGlobales),
                  },
                },
              ],
            },
      ...
      for (const fondo of fondosParaReset) {
        try {
          await this.resetearFondoIndividual(fondo, config);
          resultados.push({
            id: fondo.id_fondo,
...
export const fondoFijoResetService = new FondoFijoResetService();
