...
showToast('Error de conexión', 'error');
  ...
  const handleDelete = async (producto: Producto) => {
    if (!confirm(`¿Estás seguro de que quieres desactivar el producto "${producto.descripcion}"?`)) return;
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        try {
          ...
          // Si falla el parseo JSON, tratamos como error de conexión
          showToast('Error de conexión', 'error');
          return;
        }
      } else {
        // No es JSON: si es 401 asumimos sesión inválida; en otro caso mostrar error genérico
        if (response.status === 401) {
          showToast('Sesión expirada. Por favor, inicia sesión nuevamente.', 'error');
