  ...
  try {
    ...
    if (Array.isArray(usersWithRoles)) {
      ...
      usersWithRoles.forEach(user => {
        if (!groupedByRole[user.role_name]) {
          groupedByRole[user.role_name] = [];
        }
        groupedByRole[user.role_name].push(user);
      });
    ...
    if (unidacdExists) {
      ...
      if (unidacdRole) {
        const [permisosCount, visibilidadCount, totalPermisos, totalModulos] = await Promise.all([
          ...
          prisma.rbac_module_visibility.count({
            where: { 
              role_id: unidacdRole.id,
              is_visible: true 
            }
          ...
          prisma.rbac_modules.count()
        ]);
        
        const porcentajePermisos = totalPermisos > 0 ? ((permisosCount / totalPermisos) * 100).toFixed(1) : '0.0';
        const porcentajeVisibilidad = totalModulos > 0 ? ((visibilidadCount / totalModulos) * 100).toFixed(1) : '0.0';
