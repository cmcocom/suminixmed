...
--
-- RESPALDO CREADO: backup_antes_modulo_entradas_cliente_2025-11-23_21-50-14.backup
-- ====================================================================

BEGIN;

-- 1. Insertar permisos para el nuevo modulo
-- La tabla rbac_permissions tiene: id, name, description, module, action, is_active, created_by, created_at, updated_at
...
BEGIN
  ...
  FOR v_role IN SELECT id, name FROM rbac_roles LOOP
    ...
    FOR v_perm IN SELECT id FROM rbac_permissions WHERE module = 'REPORTES_ENTRADAS_CLIENTE' LOOP
      -- Insertar asignacion si no existe
      INSERT INTO rbac_role_permissions (id, role_id, permission_id, granted, granted_at, granted_by, created_at, updated_at)
      VALUES (
        gen_random_uuid(),
        v_role.id,
        v_perm.id,
...
BEGIN
  FOR v_role IN SELECT id, name FROM rbac_roles LOOP
    INSERT INTO rbac_module_visibility (id, role_id, module, visible, created_at, updated_at)
    VALUES (
      gen_random_uuid(),
      v_role.id,
