...
// GET - Obtener entradas consolidadas por cliente, categoría o producto
  ...
  try {
    ...
    if (process.env.DEBUG_BYPASS_CONSOLIDADO === '1') {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('[CONSOLIDADO-ENTRADAS] DEBUG auth bypass activado');
      }
    } else {
    ...
    // Verificar permisos RBAC para REPORTES_ENTRADAS_CLIENTE
    if (process.env.DEBUG_BYPASS_CONSOLIDADO !== '1') {
      ...
      if (!hasPermission) {
        return NextResponse.json(
          {
            error: 'Acceso denegado - Permisos insuficientes para reportes de entradas por cliente',
            code: 'RBAC_DENIED',
            requiredPermission: 'REPORTES_ENTRADAS_CLIENTE:LEER',
          },
          { status: 403 }
        );
      }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[CONSOLIDADO-ENTRADAS] Iniciando (agruparPor=', agruparPor, ')', {
        fechaInicio,
        fechaFin,
        clienteId,
        categoriaId,
    ...
    function buildRangeAndFilters() {
      ...
      if (clienteId) {
        ...
        conditions.push(`e.proveedor_id = $${params.length}`);
      }
    ...
    // ----------------------------------------
    // AGRUPAR POR CLIENTE/PROVEEDOR (agregación en DB)
    // ----------------------------------------
    if (agruparPor === 'cliente') {
      const bf = buildRangeAndFilters();
      const sql = `
        SELECT
          e.proveedor_id AS cliente_id,
          COALESCE(pr.nombre, 'Sin proveedor') AS cliente_nombre,
        ...
        AND e.fecha_creacion BETWEEN $1::timestamp AND $2::timestamp
        ${bf.conditions.length ? 'AND ' + bf.conditions.join('\n        AND ') : ''}
        GROUP BY e.proveedor_id, pr.nombre, i.clave, i.nombre, um.nombre, um.clave
        ORDER BY e.proveedor_id
      `;
      ...
      rows.forEach((r) => {
        const cid = r.cliente_id || 'sin_proveedor';
        if (!map[cid])
    ...
    if (agruparPor === 'producto') {
      const bf = buildRangeAndFilters();
      const params = bf.params;
      // Obtenemos filas detalladas: entrada + partida + inventario + proveedor
      const sql = `
        SELECT
          e.folio AS folio,
          e.serie AS serie,
        ...
        AND e.fecha_creacion BETWEEN $1::timestamp AND $2::timestamp
        ${bf.conditions.length ? 'AND ' + bf.conditions.join('\n        AND ') : ''}
        ORDER BY i.id, e.fecha_creacion DESC
      `;
      ...
      const resultado = Object.values(map).map((g: any) => ({
        ...
        entradas: g.entradas,
        total_entradas: g.entradas.length,
    ...
    console.error('[CONSOLIDADO-ENTRADAS] Error:', error);
    return NextResponse.json(
      {
