...
function ProductosPage() {
  ...
  const handleDeleteProduct = async (producto: Producto) => {
    ...
    try {
      const response = await api.del(`/api/inventario/${producto.id}`);
  ...
  // Filtrado y paginación - SIEMPRE muestra todos los productos sin importar existencia
  const productosFiltrados = (() => {
    ...
    // Si está marcado "mostrar todos" y no hay búsqueda, mostrar todos los productos
    if (showAll && debouncedSearchTerm.trim() === '') {
      return todosLosProductos;
    }

    // Si hay búsqueda, filtrar TODOS los productos (sin filtro de existencia)
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.descripcion.trim()) {
      ...
      errors.descripcion = 'La descripción no puede exceder 150 caracteres';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url =
        modalMode === 'create' ? '/api/inventario' : `/api/inventario/${selectedProducto?.id}`;

      const response =
        modalMode === 'create' ? await api.post(url, formData) : await api.put(url, formData);
      if (!response.ok) {
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <label htmlFor="search" className="block text-sm font-medium text-gray-700 mb-1">
                Buscar por nombre, descripción, proveedor o estado
              </label>
              <input
                type="text"
                id="search"
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-blue-100 text-sm">
                        ...
                        {modalMode === 'edit' && 'Actualiza la información del producto'}
                        {modalMode === 'view' && 'Información detallada del producto'}
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  <div className="space-y-6">
                        ...
                        {modalMode !== 'view' && (
                          <div className="flex items-center justify-center">
                            <div className="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-200 w-full">
                              ...
                              >
                                {formData.activo ? 'âœ… Activo' : 'âŒ Inactivo'}
                              </span>
                            </div>
                          </div>
                        )}
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <p className="text-sm text-gray-500 italic">
                        Los números de lote, cantidades y fechas de vencimiento se gestionan a
                        través del módulo de Entradas de Inventario.
                      </p>
                    </div>
...
export default function ProtectedProductosPage() {
  console.log('ðŸ” [PRODUCTOS PAGE] Inicializando con módulo: CATALOGOS_PRODUCTOS');
  return (
