...
export default function ProveedoresPage() {
  ...
  const fetchProveedores = useCallback(async () => {
    try {
      const response = await api.get(`/api/proveedores?limit=100`);
      if (response.ok) {
  ...
  const validateEmailUnique = async (email: string, proveedorId?: string): Promise<boolean> => {
    ...
    try {
      ...
      const response = await api.post('/api/proveedores/validate-email', requestBody);
  ...
  const handleDelete = async (proveedor: Proveedor) => {
    ...
    try {
      const response = await api.del(`/api/proveedores/${proveedor.id}`);
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-black transition-all duration-150 ${modalMode === 'view' ? 'bg-gray-50' : 'border-gray-300'}`}
                            disabled={modalMode === 'view'}
                            placeholder="Ej: 30 días, Contado, 15/30 días"
                          />
                        </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-shrink-0">
                          {formData.imagen || selectedProveedor?.imagen ? (
                            <div className="relative group">
                              <Image
                                src={formData.imagen || selectedProveedor?.imagen || ''}
                        ...
                        {modalMode !== 'view' && (
                          <div className="flex-1">
                            <input
                              ...
                              name="imagen"
                              accept="image/*"
                              title="Seleccionar archivo de imagen para el proveedor"
                              aria-label="Seleccionar archivo de imagen para el proveedor"
                              onChange={async (e) => {
