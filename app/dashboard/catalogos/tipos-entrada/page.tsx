...
export default function TiposEntradaPage() {
  ...
  const [editingTipo, setEditingTipo] = useState<TipoEntrada | null>(null);
  ...
  const fetchTipos = async () => {
    try {
      setLoading(true);
      const response = await apiFetch('/api/tipos-entrada');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTipo ? `/api/tipos-entrada/${editingTipo.id}` : '/api/tipos-entrada';
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Estás seguro de eliminar este tipo de entrada?')) return;

    try {
      const response = await apiFetch(`/api/tipos-entrada/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleEdit = (tipo: TipoEntrada) => {
    setEditingTipo(tipo);
    setFormData({
      codigo: tipo.codigo,
      nombre: tipo.nombre,
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Gestiona los tipos de movimientos de entrada de inventario
          </p>
        </div>
        <button
          ...
          <tbody className="divide-y divide-gray-200">
            {tipos.map((tipo) => (
              <tr key={tipo.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm text-gray-900">{tipo.orden}</td>
                <td className="px-6 py-4 text-sm text-gray-900 font-mono">{tipo.codigo}</td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 rounded-t-lg">
              <h3 className="text-xl font-bold text-white">
                {editingTipo ? 'Editar Tipo de Entrada' : 'Nuevo Tipo de Entrada'}
              </h3>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Descripción opcional del tipo de entrada"
                  />
                </div>
